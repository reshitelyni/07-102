...
impl Actor {
    pub fn new(args: flags::ArcArgs) -> Actor {
        let conn =
            sql3drv::Sql3Connection::open_path(format!("{}/arccal.sql3", args.prefix).as_str())
...
impl Actor {
    fn run_song(&mut self, args: flags::SongArgs) {
        match args.action {
            flags::SongArgsAction::Ls => {
                let songs = self
                    .srv
                    .query_song(None, None, None, None, None)
                    .expect("query song failed");
                songs.iter().for_each(|x| println!("{x}"));
            }
            flags::SongArgsAction::Add {
                name,
                pack,
                level,
                constant,
            ...
            flags::SongArgsAction::Update {
                ...
                assert_eq!(
                    self.srv
                        .update_song(id, None, None, None, constant)
            ...
            flags::SongArgsAction::Delete { name, level, pack } => {
                let id = self.match_song(name, pack, level);
                assert_eq!(self.srv.delete_song(id).expect("delete song failed"), 1);
            }
            flags::SongArgsAction::Search {
                name,
                pack,
                level,
                constant,
                ...
                self.srv
                    .query_song(
                        name,
                        pack,
                        level.map(|level| util::level_to_str(&level).to_owned()),
                        constant,
                        difficulty.map(|difficulty| (difficulty, plus)),
                    )
                    .expect("no songs")
                    .iter()
                    .for_each(|x| println!("{x}"));
            }
            flags::SongArgsAction::Alias {
                origin,
                alias,
                pack,
                delete: _,
                ...
                let id = self
                    .srv
                    .query_song(Some(origin), pack, None, None, None)
                    .expect("no such song");
                if id.len() >= 2 {
                    panic!("ambiguous songs");
                }
                ...
                assert_eq!(
                    self.srv.insert_alias(id, alias).expect("add alias failed"),
                    1
                );
            }
        }
    ...
    fn run_score(&mut self, args: flags::ScoreArgs) {
        match args.action {
            flags::ScoreArgsAction::Ls {
                limit,
                sort: _,
                reverse,
            } => {
                self.srv
                    .query_score(limit.unwrap_or(30) as usize, reverse)
                    .expect("query failed")
                    .iter()
                    .for_each(|x| println!("{x}"));
            }
            flags::ScoreArgsAction::Add {
                name,
                pack,
                level,
                score,
            ...
            flags::ScoreArgsAction::Potential { potential } => match potential {
